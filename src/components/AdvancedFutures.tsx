...
const AdvancedFutures = ({ metrics, warnings, projections, idealNarrative, shadowNarrative, idealHighlights, shadowHighlights }: AdvancedFuturesProps) => {
  ...
  const idealFuture = {
    ...
    description: idealNarrative || (avg > 60
    ...
    highlights: idealHighlights || defaultIdealHighlights,
  ...
  const shadowFuture = {
    ...
    description: shadowNarrative || (avg < 40
    ...
    highlights: shadowHighlights || defaultShadowHighlights,
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center gap-4 mb-4">
                <div className="text-center">
                  <div className={`font-display text-2xl font-black ${future.textClass}`}>{Math.round(future.score)}</div>
                  <div className="text-[10px] text-muted-foreground font-body">Now</div>
                </div>
                {idx === 0 ? <TrendingUp className="w-5 h-5 text-success" /> : <TrendingDown className="w-5 h-5 text-destructive" />}
                <div className="text-center">
                  <div className={`font-display text-2xl font-black ${future.textClass}`}>{Math.round(future.year10Score)}</div>
              ...
              <ul className="space-y-2">
                {future.highlights.map((h, i) => (
                  <motion.li key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.3 + i * 0.08 }} className="flex items-start gap-2 text-sm font-body">
